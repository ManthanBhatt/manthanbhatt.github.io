...
(function ($) {
	...
	$(window).on('load', function () {
		...
		//debs_obrien manthanbhatt
		fetch('https://dev.to/api/articles?username=manthanbhatt').then(response => response.json())
			.then(data => {
				console.log(data);
				data.forEach(itm => {
					const elem = document.getElementById('blog');
					const htmlString = `
						<div class="card border-radius-10">
							<img src="`+ itm.social_image + `" />
							<div class="card-body p-4 bg-dark">
								<h4 class="text-dark mb-3">`+itm.title+`</h4>
								<p>
									`+itm.description+`
									<a class="btn btn-light" href='`+itm.canonical_url+`' target="blank">Read more</a>
								</p>
							</div>
						</div>
					`;
